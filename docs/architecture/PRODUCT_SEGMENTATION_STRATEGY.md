...
*   **Inventory & SCM:** Warehousing, Stock levels, Valuation. *(Required: Strict tenant isolation and row-level scoping).*
...
*Spaghetti code is strictly prohibited.* Modules must never execute direct database writes against another module's schema. Integrations must use **Domain Events**.
...
| **Abren People** | *Add-on to any tier* | **HR & Payroll** | "Native payroll synced to the ledger." |
...
*   **Frontend Routing:** The client reads the `SubscriptionContext` upon login and trims the 220px Left-Hand Navigation Rail. If the tenant lacks the *Operations* package, the entire "Procurement", "Inventory", and "Warehousing" UI paths simply do not exist in their DOM.
